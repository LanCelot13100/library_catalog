...
class BookRepository:
    ...
    def add_book(self, book_data: dict):
        ...
        if extra:
            ...
            logger.warning(f"⚠️ No OpenLibrary match found for '{book_data['title']}'")
    ...
    def update_book(self, book_id: int, book_data: dict):
        logger.info(f"✏️ Updating book with ID={book_id}")
        books = self.get_books()
        for book in books:
            if book["id"] == book_id:
                ...
                return book
        logger.warning(f"⚠️ Book with ID={book_id} not found for update")
        return None
    ...
    def delete_book(self, book_id: int):
        ...
        if len(updated) == len(books):
            logger.warning(f"⚠️ Book with ID={book_id} not found for deletion")
            return None
